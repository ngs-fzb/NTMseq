...
###Version###
version="1.0.2" 
#Added -p to create temporary directory
#Added thread parameter
#Added temporary folder option to circumvent no space on device problem
...
do
	...
	then
		...
		shovill --R1 $fastq --R2 $(echo $fastq | sed "s/$Fw$filetype$/$Rv$filetype/1") --outdir $PATH_output/$SampleName --gsize $genome_size --depth $cov --trim --assembler $ass --tmpdir $PATH_tmp --cpus $cpu
