...
###Version###
version="1.1.0"
#Added minimum identity parameter
...
#It is advised just to run AMRfinder with default values, and be very careful if you specify ident_min and coverage_min --> if you want to use these values, uncomment line 124 and comment line 125
...
Help()
{
   ...
   echo "-m		ident_min=0.5 #<0-1; Default: 0.9> Minimum identity for a blast-based hit (Methods BLAST or PARTIAL). -1 means use a curated threshold if it exists and 0.9 otherwise. Setting this value to something other than -1 will override any curated similarity cutoffs."
   echo ""
...
while getopts ":hi:o:c:d:e:m::v" option; do #:h does not need argument, f: does need argument
   case $option in
      h) # display Help
         Help
         exit;;
...
do
	...
	then
		...
		#amrfinder -n $fasta --threads $cpu --name $SampleName -o $PATH_output/$SampleName".txt" --plus -d $db_AMRfinder
	fi
done
