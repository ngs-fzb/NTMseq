...
version="1.2.1" 
#Version changes
# Complemented parameters check

#Previous version changes
# Made conda environment path detection portable using CONDA_PREFIX.
# Changed the conda env parser for case you give a krona db location
# Added chmod -R 755 $db_kraken2 Otherwise sometimes the error "folder does not contain necessary file taxo.k2d 
...
if [[ -z "$PATH_input" ]] || [[ -z "$PATH_output" ]] || [[ -z "$db_kraken2" ]] || [[ -z "$db_krona" ]] || [[ -z "$cpu" ]]; then
    echo "Please provide all required arguments (-i PATH_input, -o PATH_output, -d db_kraken2, -k db_krona, -c CPU)! Use starter_kraken2.sh -h for help on syntax."
    exit 1
fi
...
mkdir -p $PATH_output
#mkdir -p $PATH_tmp
chmod -R 755 $db_kraken2 #Otherwise sometimes the error "folder does not contain necessary file taxo.k2d 
...
then
	...
	PATH_conda_env="$CONDA_PREFIX"
	echo "PATH conda: $PATH_conda_env"
    mkdir -p "$PATH_conda_env/opt/krona"
...
then
	...
	do
		...
		then
			...
			kraken2 --threads $cpu --db $db_kraken2 --output $PATH_output/$SampleName.kraken --report $PATH_output/$SampleName.report $input
			awk -F '\t' '{print $2"	"$3}' $PATH_output/$SampleName.kraken > $PATH_output/$SampleName.krona
			ktImportTaxonomy -o $PATH_output/$SampleName.krona.html $PATH_output/$SampleName.krona
		fi
	done
